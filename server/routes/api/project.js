...
authRouter.delete("/:projectId/col/:colId/item/:itemId", (req, res) => {
  ...
  Project.findById(projectId).then(project => {
    ...
    if (colId == -1)
      return res.status(400).send({ msg: "Column is not exist" });

    console.log(itemId);
    console.log(project.cols[colIdx].items.length);
    project.cols[colIdx].items = project.cols[colIdx].items.filter(
...
authRouter.post("/:projectId/col/:colId/item/:itemId/modify", (req, res) => {
  ...
  Project.findById(projectId)
    .then(project => {
      ...
      if (itemIdx == -1)
        return res.status(400).send({ msg: "Item is not exist." });
